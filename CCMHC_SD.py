...
def transfer_loss(region1, region2, attention, time_slot):
    ...
    for tidx in range(attention.shape[1]):  ###每个目标区域进行迁移
        ...
        for i in range(time_slot.shape[0]):
            ...
            match_corr = np.abs(attention[slot_idx, tidx, 1])
            if match_corr < 0.1:
                continue
            else:
                # print(slot_idx)
...
def train(XS, XT, YT, iter_num, source_city, target_city, w):
    ...
    attention = np.load('./{}-{}_filter_absP.npy'.format(source_city, target_city))  # size=(time_slot,225,2)
    train_loss_pre = []
...
def get_pre(test_X, target_label, para, source_city, target_city):
    ...
    print("average_preshape:",average_pre.shape);
    count = 0
    ###将空区域赋0，预测值为负赋0
    for re_id in range(225):
        if para[re_id][1] == 0:
            ...
            for i in range(len(temp)):
                ...
                elif attention[(i + seq_len) % 24, re_id,1] < 0:
                    average_pre[i, re_id] = 1 - average_pre[i, re_id]
    ## Denormalize
    para_min = para[:, 0]
    para_max = para[:, 1]
    rever_pre = []  # 反归一化的预测值
...
def data_preprocessing(XS, YS, source_test_x, source_test_y, XT, YT, test_X, test_Y,source_city,target_city):
    ...
    XT = XT.reshape(-1, batch_size, seq_len, 1, 225)  # (batch,  batch_size,  seq ,1  ,W,  H, )
    XT = torch.tensor(XT)
    YT = YT.reshape(-1, batch_size, 225)
    YT = torch.tensor(YT)  # (batch,  batch_size, 15, 15)

    test_X = test_X.reshape(-1, batch_size, seq_len, 1, 225)  # (batch,  batch_size,  seq .1  ,W,  H, )
    test_X = torch.tensor(test_X)
    test_Y = test_Y.reshape(-1, batch_size, 225)
    ...
    attention = np.load('./{}-{}_filter_absP.npy'.format(source_city, target_city))#24 255 2

    a=0
    b=0
    #对称化
    for netidx in range(XT.shape[4]):
        for timeidx1 in range(XT.shape[0]):
            if(attention[timeidx1%24,netidx,1]<0):
                XT[timeidx1,:,:,:,netidx]=1-XT[timeidx1,:,:,:,netidx]
                YT[timeidx1,:,netidx]=1-YT[timeidx1,:,netidx]
                a+=1
        for timeidx2 in range(test_X.shape[0]):
            if (attention[timeidx2% 24, netidx, 1] < 0):
                test_X[timeidx2, :, :, :, netidx] = 1 - test_X[timeidx2, :, :, :, netidx]
                test_Y[timeidx2, :, netidx] = 1 - test_Y[timeidx2, :, netidx]
                b += 1
    print("对称化:{}次".format(a))
    XT=XT.reshape(-1, batch_size, seq_len, 1, 15,15)
    YT=YT.reshape(-1, batch_size,  15, 15)
    print("对称化:{}次".format(b))
    test_X = test_X.reshape(-1, batch_size, seq_len, 1, 15, 15)
...
if __name__ == '__main__':
    ...
    train_X, train_Y, test_X, test_Y, label, normed_label, source_test_para, source_train_para = load_data_1h(
        train_size=train_size, seq_len=seq_len, city_name=source_city)
    target_train_X, target_train_Y, target_test_X, target_test_Y, target_label, target_normed_label, target_para, train_target_para = load_data_1h(
        train_size=train_size, seq_len=seq_len, city_name=target_city)
    #print("target_para:",target_para.shape, "\ttrain_target_para:",train_target_para.shape)#225x2
    ### npy to tensor
    train_X, train_Y, test_X, test_Y, target_train_X, target_train_Y, target_test_X, target_test_Y \
        = data_preprocessing(train_X, train_Y, test_X, test_Y, target_train_X, target_train_Y, target_test_X,
                             target_test_Y,source_city,target_city)

    print("train_X.shape:", train_X.shape)
    print("train_Y.shape:", train_Y.shape)
    print("target_normed_label.shape:",target_normed_label.shape)
    ## train
    train(train_X, target_train_X, target_train_Y, 50, source_city, target_city, w=0.5)
