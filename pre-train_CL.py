...
def adjust_learning_rate(optimizer,epoch, lr):
    ...
    for param_group in optimizer.param_groups:
        param_group['lr'] = lr
def train_source(train_X, train_Y, test_X, test_Y,source_city, iternum , state):
    print("pretrain {} {}".format(source_city,state))
    ...
    torch.save(conv_lstm.state_dict(), './model/pre_trained/{}_CL_{}.pkl'.format(source_city,state))

def load_data_neg(Xtrain,Ytrain,Xtest,Ytest,test_para,train_para):
    #get negative data fold in half
    Xtrain_neg = Xtrain.reshape(-1,seq_len,225).copy()
    Ytrain_neg = Ytrain.reshape(-1,225).copy()
    Xtest_neg = Xtest.reshape(-1,seq_len,225).copy()
    Ytest_neg = Ytest.reshape(-1, 225).copy()
    count1=0
    count2=0
    for i in range(225):
        # if(train_para[i,1]==0):
        ...
        #     count1+=1
        # else:
        for j in range(Xtrain_neg.shape[0]):
            Xtrain_neg[j,:,i] = np.median(Xtrain_neg[j,:,i])- Xtrain_neg[j,:,i]
    ...
    for i in range(225):
        ...
        #     count2+=1
        # else:
            Xtest_neg[:, :, i] = np.min([1,2*np.mean(Xtest_neg[:,:,i])]) - Xtest_neg[:, :, i]
            Ytest_neg[:, i] = np.min([1,2*np.mean(Ytest_neg[:,i])]) - Ytest_neg[:,i]
    Xtrain_neg = Xtrain_neg.reshape(-1, seq_len, 15,15)
    Ytrain_neg = Ytrain_neg.reshape(-1, 15,15)
    Xtest_neg = Xtest_neg.reshape(-1, seq_len, 15,15)
    Ytest_neg = Ytest_neg.reshape(-1, 15,15)
    print(count1)
    print(count2)
    return Xtrain_neg,Ytrain_neg,Xtest_neg,Ytest_neg
if __name__ == '__main__':
    ...
    source_city='NJ'
    # for source_city,train_size in [["NJ",30],["HK",30],["SH",24]]:
    ...
    print("train_X:",train_X.shape) #570 6 15 15
    print("train_Y:", train_Y.shape)#570 15 15
    print("test_X:",test_X.shape )  #330 6 15 15
    print("test_Y:",test_Y.shape )  #330 15 15
    print("label:",label.shape )    #330 225
    print("normed_label:",normed_label.shape )#330 225
    print("test_para:",test_para.shape )    #225 2
    print("train_para:",train_para.shape )  #225 2

    train_X_neg,train_Y_neg,test_X_neg,test_Y_neg=load_data_neg(train_X,train_Y,test_X,test_Y,test_para,train_para)
    for i in range(225):
        print("{}:\ttrain_Y:{}\ttrain_Y_neg:{}".format(i, train_Y[8, int(i/15),i%15], train_Y_neg[8, int(i/15),i%15]))
    train_source(train_X, train_Y, test_X, test_Y,source_city ,iternum=50,state="pos")
    # train_source(train_X_neg,train_Y_neg,test_X_neg,test_Y_neg, source_city, iternum=50, state="neg")
